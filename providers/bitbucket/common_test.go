...
func LoadAssets(c *C) {
	responder1 := test.ResponderByFile(c, filepath.Join("assets", "1.json"))
	httpmock.RegisterResponder("GET", "https://api.bitbucket.org/2.0/repositories?pagelen=100", responder1)

	responder2 := test.ResponderByFile(c, filepath.Join("assets", "2.json"))
	...
	responder3 := test.ResponderByFile(c, filepath.Join("assets", "3.json"))
	...
	responder4 := test.ResponderByFile(c, filepath.Join("assets", "4.json"))
	...
	responderEmpty := test.ResponderByFileAndStatus(c, filepath.Join("assets", "no_results.json"), 304)
